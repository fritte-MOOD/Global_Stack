...
function CommunitiesWindowContent({ groups }: { groups: DesktopGroup[] }) {
  return (
    <div className="p-4 space-y-4">
      ...
      <button className="w-full py-2 px-4 text-sm font-medium text-brand-700 border border-dashed border-brand-300 rounded-lg hover:bg-brand-50 transition-colors cursor-pointer">
...
function AccountWindowContent({ user }: { user: DesktopUser | null }) {
  if (!user) {
    return (
      <div className="p-4 text-sm text-brand-700">
        Not logged in
      </div>
    );
  }
    ...
    <div className="p-4 space-y-4">
      <div className="flex items-center gap-4">
        <div className="w-16 h-16 rounded-full bg-brand-200 flex items-center justify-center">
          <User className="size-8 text-brand-600" />
        </div>
        <div>
          <div className="font-medium text-lg text-brand-950">{user.name}</div>
          <div className="text-sm text-brand-600">@{user.username}</div>
          {user.isDemo && (
            <span className="inline-block mt-1 text-xs bg-brand-200 text-brand-700 px-2 py-0.5 rounded">
              Demo Account
            </span>
          )}
        </div>
      ...
      <div className="border-t border-brand-100 pt-4 space-y-3">
        <button className="w-full py-2 px-4 text-sm font-medium text-brand-700 border border-brand-200 rounded-lg hover:bg-brand-50 transition-colors cursor-pointer text-left">
          Edit Profile
        </button>
        <button className="w-full py-2 px-4 text-sm font-medium text-brand-700 border border-brand-200 rounded-lg hover:bg-brand-50 transition-colors cursor-pointer text-left">
          Change Password
