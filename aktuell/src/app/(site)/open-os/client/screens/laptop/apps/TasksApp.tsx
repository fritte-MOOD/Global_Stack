...
export default function TasksApp({ data, groupIds, allGroups }: Props) {
  ...
  if (tasks.length === 0) {
    return (
      <div className="flex items-center justify-center h-full min-h-[200px]">
        <span className="text-xs text-brand-500">No tasks in this group.</span>
      </div>
    );
  }
  ...
  return (
    <div className="p-4 space-y-4">
      {open.length > 0 && (
        <div>
          <div className="text-xs font-semibold text-brand-700 mb-2 px-1">Open ({open.length})</div>
      ...
      {done.length > 0 && (
        <div>
          <div className="text-xs font-semibold text-brand-500 mb-2 px-1">Done ({done.length})</div>
  ...
  return (
    <div className="flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-brand-50 transition-colors">
      {task.done ? (
        ...
        <Circle className="size-4 text-brand-300 shrink-0" />
      )}
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          <span className={`text-xs ${task.done ? "line-through text-brand-500" : "text-brand-950"}`}>{task.title}</span>
        </div>
