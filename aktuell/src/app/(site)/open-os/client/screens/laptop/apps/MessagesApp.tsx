...
export default function MessagesApp({ data, groupIds, allGroups }: Props) {
  const messages = data.messages.filter((m) => groupIds.includes(m.groupId));
          ...
          <div className="space-y-1.5 mt-2">
            {items.map((msg) => (
              <div key={msg.id} className="flex gap-3 px-3 py-2 rounded-lg hover:bg-brand-50 transition-colors">
                <div className="w-7 h-7 rounded-full bg-brand-200 flex items-center justify-center shrink-0 text-[10px] font-semibold text-brand-700">
...
function GroupHeader({ group }: { group: { name: string; color: string } }) {
  return (
    <div className="flex items-center gap-2 px-1">
      ...
      <span className="text-xs font-semibold text-brand-700">{group.name}</span>
    </div>
  );
}
...
function groupByGroup<T extends { groupId: string }>(
  items: T[],
  _key: string,
  allGroups: (DemoGroup & { depth: number })[],
  ...
  for (const item of items) {
    const arr = map.get(item.groupId) ?? [];
    arr.push(item);
    map.set(item.groupId, arr);
  }
